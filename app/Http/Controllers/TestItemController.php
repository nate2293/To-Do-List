<?php

namespace App\Http\Controllers;

use App\Http\Requests\StoreTestItemRequest;
use App\Http\Requests\UpdateTestItemRequest;
use App\Models\TestItem;

class TestItemController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreTestItemRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(TestItem $testItem)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(TestItem $testItem)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateTestItemRequest $request, TestItem $testItem)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(TestItem $testItem)
    {
        //
    }
}
